...
@interface VCFridgeList ()

@end

@implementation VCFridgeList
...
- (void) initialize {
    ...
    self.fridgeLogItems = [[NSMutableArray alloc] init];
    self.tableView.dataSource =self;
    self.tableView.delegate = self;
    if([[UserInfo shared].mAccount.mPermissionModel checkPermission:@"fridge" withCategory:@"log" withFunction:@"capture"]){
...
- (void) textViewTapped:(UITapGestureRecognizer *)tapGesture {
    ...
    if([[UserInfo shared].mAccount checkSignature]) {
        AssignTableCellModel* model = [self.fridgeAssignModels objectForKey:indexString];
        if(model != nil){
...
- (void) loadData {
    ...
    if(self.fridgeLogItems == nil) {
        self.fridgeLogItems = [[NSMutableArray  alloc ] init];
    }
    if(self.fridgeAssignModels == nil) {
        self.fridgeAssignModels = [[NSMutableDictionary  alloc ] init];
    }
    ...
    [[NetworkParser shared] serviceGetFridgeInfo:^(id responseObject, NSString *error) {
        if(error == nil) {
            ...
            for(int i=0; i<self.infoModel.mAssigns.count; i++) {
                ...
                self.fridgeAssignModels[indexString] = model;
