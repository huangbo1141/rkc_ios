...
@interface VCOilItemAreaScan ()<QRCodeReaderDelegate>

@end

@implementation VCOilItemAreaScan
...
- (void)viewDidLoad {
    ...
    AssignModel* assignModel= (AssignModel*) self.logModel.mAssignModel;
    if(assignModel != nil) {
...
- (IBAction)nextAction:(id)sender {
    if([self confirmData]) {
        if([[UserInfo shared].currentLogic isEqualToString:@"oil"]) {
            [Global switchScreen: self withStoryboardName:@"Oil"  withControllerName:@"VCOilInfo"];
        }
    }
}
    ...
    [reader setCompletionWithBlock:^(NSString *resultAsString) {
        if(resultAsString != nil){
            ...
            if(params.count == 2) {
                ...
                [[NetworkParser shared] serviceGetOilItem:itemId withCompletionBlock:^(id responseObject, NSString *error) {
                    if(error == nil) {
...
- (void) search:(NSString*) key withAreaId:(NSString*) areaId{
    ...
    [[NetworkParser shared] serviceGetOilItems:key withAreaId:areaId withCompletionBlock:^(id responseObject, NSString *error) {
        if(error == nil) {
