...
@interface VCServiceList ()

@end

@implementation VCServiceList
- (void)viewDidLoad {
    [super viewDidLoad];
    [self initialize];
}
...
- (void) initialize {
    ...
    [UserInfo shared].currentLogic = @"service";
    ...
    self.serviceLogItems = [[NSMutableArray alloc] init];
...
- (void) loadData {
    ...
    if(self.serviceLogItems == nil) {
        self.serviceLogItems = [[NSMutableArray  alloc ] init];
    }
    [Global showIndicator:self];
    [[NetworkParser shared] serviceGetServiceInfo:^(id responseObject, NSString *error) {
        if(error == nil) {
