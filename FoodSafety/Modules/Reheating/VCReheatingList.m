...
@interface VCReheatingList ()

@end

@implementation VCReheatingList
...
- (void) initialize {
    ...
    [UserInfo shared].currentLogic = @"reheating";
    ...
    self.reheatingLogItems = [[NSMutableArray alloc] init];
    self.tableView.dataSource =self;
    self.tableView.delegate = self;
    if([[UserInfo shared].mAccount.mPermissionModel checkPermission:@"reheating" withCategory:@"log" withFunction:@"capture"]){
...
- (void) loadData {
    ...
    if(self.reheatingLogItems == nil) {
        self.reheatingLogItems = [[NSMutableArray  alloc ] init];
    }
    [Global showIndicator:self];
    [[NetworkParser shared] serviceGetReheatingInfo:^(id responseObject, NSString *error) {
        if(error == nil) {
