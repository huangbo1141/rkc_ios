#ifndef CLEARSHIELD_MACRO_H_DEFINED
#define CLEARSHIELD_MACRO_H_DEFINED

#ifdef DEBUG
#define MyNSLog(s, ...) NSLog( @"[%@ %s(%d)] %@", [[NSString stringWithUTF8String:__FILE__] lastPathComponent], __PRETTY_FUNCTION__, __LINE__, [NSString stringWithFormat:(s), ##__VA_ARGS__] )
#else
#define MyNSLog(s, ...)
#endif
...
#define UDRemove(x)             [[NSUserDefaults standardUserDefaults] removeObjectForKey:(x)]
...
#define CGHexColor(c) 						[[UIColor colorWithRed:((c>>24)&0xFF)/255.0 green:((c>>16)&0xFF)/255.0 blue:((c>>8)&0xFF)/255.0 alpha:((c)&0xFF)/255.0] CGColor]
